...
if halfCos
    xVecTotal = nonLinspace(0, c, NptsTop, 'cos'); % Cosine spacing (D. Jayaraman)
else
...
%     % Derivative that I refuse to take myself.
%     dyCamF_dxSYM = diff(sym(yCam_frnt));
%     dyCamB_dxSYM = diff(sym(yCam_back));
%     dyCamF_dx = matlabFunction(dyCamF_dxSYM); % converting back to anon.
%     dyCamB_dx = matlabFunction(dyCamB_dxSYM);
